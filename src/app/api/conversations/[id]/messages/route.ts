import { NextRequest, NextResponse } from "next/server"
import { auth } from "@/lib/auth"
import { getMessagesByConversation, createMessage } from "@/services/message"
import { getConversationById } from "@/services/conversation"
import { processMessageMentions, triggerNotification } from "@/services/notification"
import { prisma } from "@/lib/prisma"

export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await auth()
  if (!session?.user?.id) {
    return NextResponse.json({ success: false, error: "Unauthorized" }, { status: 401 })
  }

  try {
    const { id } = await params
    const { searchParams } = new URL(request.url)
    const limit = searchParams.get("limit") ? parseInt(searchParams.get("limit")!) : undefined
    const before = searchParams.get("before") ?? undefined

    const messages = await getMessagesByConversation(id, { limit, before })
    return NextResponse.json({ success: true, data: messages })
  } catch (error) {
    console.error("Error getting messages:", error)
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 })
  }
}

export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const session = await auth()
  if (!session?.user?.id) {
    return NextResponse.json({ success: false, error: "Unauthorized" }, { status: 401 })
  }

  try {
    const { id } = await params
    const conversation = await getConversationById(id)
    if (!conversation) {
      return NextResponse.json({ success: false, error: "Conversation not found" }, { status: 404 })
    }

    const isMember = conversation.members.some((m) => m.userId === session.user.id)
    if (!isMember) {
      return NextResponse.json({ success: false, error: "Forbidden" }, { status: 403 })
    }

    const body = await request.json()
    const { content, fileUrls } = body

    if (!content?.trim() && (!fileUrls || fileUrls.length === 0)) {
      return NextResponse.json({ success: false, error: "Content or file is required" }, { status: 400 })
    }

    const message = await createMessage({
      workspaceId: conversation.workspaceId,
      conversationId: id,
      senderId: session.user.id,
      content: content?.trim() ?? "",
      fileUrls: fileUrls as string[] | undefined,
    })

    try {
      const sender = await prisma.user.findUnique({
        where: { id: session.user.id },
        select: { name: true },
      })
      // Notify other conversation participants of a received message.
      for (const m of conversation.members) {
        if (m.userId === session.user.id) continue
        await triggerNotification({
          userId: m.userId,
          workspaceId: conversation.workspaceId,
          type: "message.received",
          category: "MESSAGE",
          priority: "NORMAL",
          title: `New message from ${sender?.name ?? "a member"}`,
          message: content.trim().slice(0, 200),
          actorId: session.user.id,
          entityId: message.id,
          entityType: "message",
          link: `/messages?conversation=${id}`,
        })
      }
      await processMessageMentions({
        content: content.trim(),
        workspaceId: conversation.workspaceId,
        senderId: session.user.id,
        senderName: sender?.name ?? "A member",
        messageId: message.id,
        conversationId: id,
        link: `/messages?conversation=${id}`,
        everyoneAllowed: false,
      })
    } catch {
      // message notifications are best-effort
    }

    return NextResponse.json({ success: true, data: message }, { status: 201 })
  } catch (error) {
    console.error("Error creating message:", error)
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 })
  }
}
